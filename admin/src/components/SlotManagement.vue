  ...
  <v-container fluid >
    ...
    <v-row no-gutters>
              <v-toolbar class="elevation-1" color="white" rounded>
                <v-toolbar-title class="font-weight-bold"><v-icon size="large" class="mr-2" theme="dark">mdi-calendar-clock</v-icon>Slots</v-toolbar-title>
                <v-spacer></v-spacer>
          ...
          <v-text-field
            ...
            <v-btn color="primary" class="ma-2" @click="actform=true, state='new'">
              ...
              <v-icon left class="mr-2">mdi-plus</v-icon>  Open
            </v-btn>
              </v-toolbar>
      </v-row>
      ...
      <SlotForm :show="actform" :state="state" :item_id="account.id" @FormEvent="FormEvent"/>
      ...
      methods: {
        ...
        getItems() {
          api.post('admin/qry',{
                table: 'bookingslots',
                type: 'query',
                query: "select * from booking_slots order by id desc"
            })
        .then(response => {
          if(response.status) {
            console.log("Data: ",response.data)
              this.itemlist = response.data.bookingslots
                this.item_count = response.data.bookingslots.length
