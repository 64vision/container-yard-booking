        ...
        <v-col cols="12" sm="12" md="12" lg="12">
            ...
            <thead>
              <tr >
                ...
                <th class="text-left hp-head" style="width: 250px;">Account</th>
            ...
            <tbody>
              <tr v-for="(item,index) in cashouts" :key="index">
                ...
                <td :class="Even(index)">{{ item.account_name}} <br/> <em>{{item.account_id}}</em>
  ...
  export default {
    ...
    methods: {
      ...
      getcashouts(){
        ...
        api.post('admin/qry', {
          type: "cashouts",
          query:"select concat(a.first_name,', ', a.last_name) as account_name, c.* from cashouts c left join accounts a on a.id=c.account_id where to_date(c.req_at::text,'YYYY-MM-DD') >= '"+this.from_date+"' and  to_date(c.req_at::text,'YYYY-MM-DD') <= '"+this.to_date+"' order by c.id desc"
        })
        .then(response => {
          console.log(response.data)
          if(response.data.status) {
