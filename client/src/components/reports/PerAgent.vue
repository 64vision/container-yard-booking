<template>
    <div>
      <v-card color="accent"> 
            ...
            <thead>
              <tr >
                <th class="text-left hp-head">Agent</th>
            ...
            <tbody>
              <tr v-for="(summary,index) in sales_summaries" :key="index">
                ...
                <td :class="Even(index)">{{ summary.per_agent }}</td>
                <td  :class="Even(index)">
        ...
        if(per_games == undefined) {
          ...
          var games = []
            for (var i = 0; i < this.Seq.length; i++) {
             per_games.reduce((res, item) =>{
                    if(item.game == this.Seq[i]){
                      games[i] = item
                    } else {
                      games[i] = {game: this.Seq[i], total: 0}
                    }
                  }, [])
            }
            return games
